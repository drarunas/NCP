...
/* TT 17678: add ability to detect touch events
   ...
   See http://blog.r-no.fr/drag-drop-dans-des-pages-web-sur-les-tablettes/ for reference
   For jQuery code (for reference only) see http://touchpunch.furf.com/
*/
...
if (!Array.prototype.indexOf)
    Array.prototype.indexOf = function (item, i) {
        ...
        //if (i < 0) i = length + i;
        if (i < 0) i = Math.max(0, length + i); //TT 15332: make sure don't exceed bounds of array in the other direction
                    ...
                    if (values && typeof values == "object") {
                        if (Object.isArray(values))
                            return results.concat(
                                values.map(toQueryPair.curry(key))
                            );
...
Element.Methods = {
    ...
    up: function (element, expression, index) {
        ...
        //CHASE: fix for up() issue in IE
        if (Prototype.Browser.IE) {
            if (Object.isNumber(expression)) return ancestors[expression];
            for (var cnt = 0; cnt < ancestors.length; cnt++) {
                if ($(ancestors[cnt]).match(expression, index))
                    return $(ancestors[cnt]);
            }
            return;
        }
        //END
...
Event.Methods = (function () {
    ...
    return {
        ...
        //TT 17678: call pointerXTouch() and pointerYTouch() instead
        ...
        //     },
        pointer: function (event) {
            return { x: pointerXTouch(event), y: pointerYTouch(event) };
        },
    ...
    //TT 17678: moved X code out of pointer() above and added support for touch event
    function pointerXTouch(event) {
        var docElement = document.documentElement,
    ...
    //TT 17678: moved Y code out of pointer() above and added support for touch event
    function pointerYTouch(event) {
        var docElement = document.documentElement,
    ...
    (function () {
        ...
        function createWrapper(element, eventName, handler) {
            ...
            var wrapper = function (event) {
                ...
                Event.extend(event);
                if (handler) handler.call(element, event); // AW Bug fix
            };
